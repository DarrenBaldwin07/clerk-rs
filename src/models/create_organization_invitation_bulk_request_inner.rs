...
impl CreateOrganizationInvitationBulkRequestInner {
	...
	pub fn validate(&self) -> Result<(), String> {
		...
		// Validate inviter_user_id
		if !input::validate_user_id(&self.inviter_user_id) {
			return Err(format!("Invalid user ID format: {}", self.inviter_user_id));
		}

		// Validate role
		if !input::validate_role(&self.role) {
			return Err(format!("Invalid role: Role cannot be empty"));
		}
