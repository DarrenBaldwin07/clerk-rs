...
impl SignUps {
	...
	/// Get the details of a specific sign up attempt given its ID
	pub async fn get_sign_up(clerk_client: &Clerk, id: &str) -> Result<crate::models::SignUp, Error<GetSignUpError>> {
		let local_var_configuration = &clerk_client.config;
		...
		if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
			...
			let local_var_entity: Option<GetSignUpError> = serde_json::from_str(&local_var_content).ok();
			let local_var_error = ResponseContent {
				status: local_var_status,
				content: local_var_content,
				entity: local_var_entity,
