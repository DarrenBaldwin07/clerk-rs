...
pub async fn create_organization_invitation_bulk(configuration: &configuration::Configuration, organization_id: &str, create_organization_invitation_bulk_request_inner: Vec<crate::models::CreateOrganizationInvitationBulkRequestInner>) -> Result<crate::models::OrganizationInvitations, Error<CreateOrganizationInvitationBulkError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/organizations/{organization_id}/invitations/bulk", local_var_configuration.base_path, organization_id=crate::apis::urlencode(organization_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    local_var_req_builder = local_var_req_builder.json(&create_organization_invitation_bulk_request_inner);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateOrganizationInvitationBulkError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
