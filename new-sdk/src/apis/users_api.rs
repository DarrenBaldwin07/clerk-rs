...
pub async fn set_user_profile_image(configuration: &configuration::Configuration, user_id: &str, file: Option<std::path::PathBuf>) -> Result<crate::models::User, Error<SetUserProfileImageError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/users/{user_id}/profile_image", local_var_configuration.base_path, user_id=crate::apis::urlencode(user_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SetUserProfileImageError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn users_get_organization_memberships(configuration: &configuration::Configuration, user_id: &str, limit: Option<f32>, offset: Option<f32>) -> Result<crate::models::OrganizationMemberships, Error<UsersGetOrganizationMembershipsError>> {
    let local_var_configuration = configuration;
