...
pub async fn delete_organization_logo(configuration: &configuration::Configuration, organization_id: &str) -> Result<crate::models::Organization, Error<DeleteOrganizationLogoError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DeleteOrganizationLogoError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn list_organizations(configuration: &configuration::Configuration, limit: Option<f32>, offset: Option<f32>, include_members_count: Option<bool>, query: Option<&str>, order_by: Option<&str>) -> Result<crate::models::Organizations, Error<ListOrganizationsError>> {
    let local_var_configuration = configuration;
...
pub async fn upload_organization_logo(configuration: &configuration::Configuration, organization_id: &str, uploader_user_id: &str, file: std::path::PathBuf) -> Result<crate::models::OrganizationWithLogo, Error<UploadOrganizationLogoError>> {
    let local_var_configuration = configuration;
    ...
    local_var_form = local_var_form.text("uploader_user_id", uploader_user_id.to_string());
